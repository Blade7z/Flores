...
import { chime, gardenChime } from './lib/chime';
...
function FlowerIcon() {
  return (
    <svg className="hint__flower" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
function App() {
  ...
  const spawnBurst = useCallback((bx: number, by: number, count: number) => {
    idRef.current += 1;
    setBursts((prev) => [...prev, { id: idRef.current, kind: 'sparkle', bx, by, count }]);
  }, []);
